...
class PyNET(nn.Module):
    ...
    def __init__(self, level, instance_norm=True, instance_norm_level_1=False, residual = False):
        super(PyNET, self).__init__()

        self.level = level
        self.residual = residual
        self.offset = 4 if residual else 0 
        ...
        self.conv_l4_d3 = ConvMultiBlock(512 + self.offset , 256, 3, instance_norm=instance_norm)
        ...
        self.conv_l3_d3 = ConvMultiBlock(256 + self.offset, 128, 5, instance_norm=instance_norm)
        ...
        self.conv_l2_d3 = ConvMultiBlock(128 + self.offset, 64, 5, instance_norm=instance_norm)
        ...
        self.conv_l1_d3 = ConvMultiBlock(64 + self.offset, 32, 5, instance_norm=False)
    ...
    def level_4(self, conv_l4_d1, conv_t4a, conv_t4b, i4):
        
        if self.residual:   
            conv_l4_d2 = torch.cat([conv_l4_d1, conv_t4a, i4], 1)
        else:
            conv_l4_d2 = torch.cat([conv_l4_d1, conv_t4a], 1)
        #print('conv_l4_d2', conv_l4_d2.shape)
    ...
    def level_3(self, conv_l3_d1, conv_t3a, conv_t3b, i3):

        if self.residual:   
            conv_l3_d2_ = torch.cat([conv_l3_d1, conv_t3a, i3], 1)
        else:
            conv_l3_d2_ = torch.cat([conv_l3_d1, conv_t3a], 1)
            
        conv_l3_d2 = torch.cat([conv_l3_d1, conv_t3a], 1)
        #print('conv_l3_d2', conv_l3_d2.shape)

        conv_l3_d3 = self.conv_l3_d3(conv_l3_d2_) + conv_l3_d2
    ...
    def level_2(self, conv_l2_d1, conv_t2a, conv_t2b, i2):

        if self.residual:   
            conv_l2_d2 = torch.cat([conv_l2_d1, conv_t2a, i2], 1)
        else:
            conv_l2_d2 = torch.cat([conv_l2_d1, conv_t2a], 1)
        #print('conv_l2_d2', conv_l2_d2.shape)
    ...
    def level_1(self, conv_l1_d1, conv_t1a, conv_t1b, i1):

        if self.residual:   
            conv_l1_d2 = torch.cat([conv_l1_d1, conv_t1a, i1], 1)
        else:
            conv_l1_d2 = torch.cat([conv_l1_d1, conv_t1a], 1)
        #print('conv_l1_d2', conv_l1_d2.shape)
    ...
    def forward(self, x):
        ...
        if self.residual:     
            i4 = F.interpolate(x,  scale_factor = 1/8, mode='bilinear')
            i3 = F.interpolate(x,  scale_factor = 1/4, mode='bilinear')
            i2 = F.interpolate(x,  scale_factor = 1/2, mode='bilinear')
            i1 = x
        else:
            i4 = i3 = i2 = i1 = None
        ...
        if self.level < 5:
            output_l4, conv_t3a, conv_t3b = self.level_4(conv_l4_d1, conv_t4a, conv_t4b, i4)
        if self.level < 4:
            output_l3, conv_t2a, conv_t2b = self.level_3(conv_l3_d1, conv_t3a, conv_t3b, i3)
        if self.level < 3:
            output_l2, conv_t1a, conv_t1b = self.level_2(conv_l2_d1, conv_t2a, conv_t2b, i2)
        if self.level < 2:
            output_l1, conv_t0 = self.level_1(conv_l1_d1, conv_t1a, conv_t1b, i1)
        if self.level < 1:
